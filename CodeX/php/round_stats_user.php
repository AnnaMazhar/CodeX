...
$result = $conn->query($sql);
 // --------------------------------------------
       ...
       while ($row = $result->fetch_row()) {
        ...
        echo "<td> $row[0] </td>";
        echo "<td> $row[1] </td>";
        // $loc = "view_user_round_subs.php?c_id=".$row[0];
        // $click = "document.location = '".$loc."'";
        // echo '<td> <button onclick = "'.$click.'" class="btn-group4" >View</button></td>';
        ?>
        <td> <button id= "<?php echo $row[0]; ?>" onclick="id_store(this.id)"  > View </button> </td>
        <?php
...
<form action="view_user_round_subs.php" method="post">
